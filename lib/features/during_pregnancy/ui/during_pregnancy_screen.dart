...
class _DPSState extends State<DuringPregnancyScreen>
  ...
  void _calc() {
    ...
    _days = diff % 7;
    _edd = _start!.add(const Duration(days: 280));
  ...
  double get _progress => ((_weeks * 7 + _days) / 280.0).clamp(0.0, 1.0);
  int get _daysLeft =>
    ...
    return ListView(
      ...
      children: [
        ...
        ...todos
            .asMap()
            .entries
            .map((e) => _CheckItem(text: e.value, idx: e.key, week: wd.week)),
        const SizedBox(height: 14),
        // Nutrition
        Container(
          padding: const EdgeInsets.all(16),
...
class _SymChipState extends State<_SymChip>
  ...
  bool get _on => appState.isSymptomLogged(widget.symptomKey);
  ...
  Widget build(BuildContext ctx) => GestureDetector(
        ...
        onTapUp: (_) {
          _c.reverse();
          appState.toggleSymptom(widget.symptomKey);
          HapticFeedback.selectionClick();
        },
...
class _CheckItem extends StatefulWidget {
  ...
  final int idx, week;
  const _CheckItem(
      {required this.text, required this.idx, required this.week});
  @override
...
class _CheckItemState extends State<_CheckItem>
  ...
  bool get _done => appState.isTodoDone(widget.week, widget.idx);
  ...
  Widget build(BuildContext ctx) => GestureDetector(
        ...
        onTapUp: (_) {
          _c.reverse();
          appState.toggleTodo(widget.week, widget.idx);
